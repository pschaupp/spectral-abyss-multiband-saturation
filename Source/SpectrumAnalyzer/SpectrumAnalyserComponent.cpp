...
void SpectrumAnalyserComponent::drawNextFrameOfSpectrum() {
    ...
    // Rolloff low frequency visualization
    int startRolloff = 10;
    int endRolloff = 80;
    float minScale = 0.4f;

    for (int i = 0; i < endRolloff; ++i) {
        float t = static_cast<float>(i - startRolloff)/static_cast<float>(endRolloff - startRolloff);
        if (t < 0) t = 0;
        float scale = log(1 + 9 * t) / log(10);
        scale = minScale + (1.0f - minScale) * scale;
...
void SpectrumAnalyserComponent::paint(juce::Graphics &g) {
    ...
    spectrumPath.startNewSubPath(0.0f, static_cast<float>(plotArea.getBottom()));

    int resolution = 10;
    for (int i = 1; i < scopeSize; i += resolution) {
        spectrumPath.lineTo(
        static_cast<float>(juce::jmap(i, 0, scopeSize - 1, 0, width)),
        juce::jmap(displayData[i], 0.0f, 1.0f,
                   static_cast<float>(plotArea.getBottom()),
                   static_cast<float>(plotArea.getY()))
        );
        if (i == 191) resolution = 4;
        if (i == 355) resolution = 2;
    }

    auto smoothPath = spectrumPath.createPathWithRoundedCorners(16.0f);

    smoothPath.lineTo(static_cast<float>(width), static_cast<float>(plotArea.getBottom()));
    smoothPath.lineTo(0.0f, static_cast<float>(plotArea.getBottom()));

    g.setColour(juce::Colour(0xFFD3D0F2));
    g.strokePath(smoothPath, juce::PathStrokeType(2));
